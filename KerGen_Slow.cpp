...
#include "KerGen_Slow.h"
#include "BaseGeoOpUtils.h"
#include "sdlp.h"
#include "CGALUtils.h"
//#include "robust-predicate-master/geompred.hpp"

KerGen_Slow::KerGen_Slow(const Mesh& hostMesh) :
	KernelExpansion(hostMesh, true) {

}

KerGen_Slow::~KerGen_Slow() {

}

void KerGen_Slow::expandKernel() {
	...
	*/
	...
	while (!tobeProcessedFaceIds.empty()) {
		...
		for (int partner = 0; ; partner++) {
			// initializations
			VertexPartners_Slow& vp = vertexPartnerships[face_id][partner];
			...
			vertexPartnerships[face_id].push_back(VertexPartners_Slow(next_partner_id, partner1_id, next_line));
		...
		faceProcess[face_id] = ProcessColor_Slow::GREEN;
...
void KerGen_Slow::initialize() {

	for (int i = 0; i < halfSpaceSet.size(); i++) {
		vertexPartnerships.push_back(vector<VertexPartners_Slow>());
		faceProcess.push_back(ProcessColor_Slow::RED);
	}

}

int KerGen_Slow::findClosestHalfSpace(double* point) {
...
int KerGen_Slow::findClosestHalfSpace(double* point, int base_plane_id) {
...
vector<int> KerGen_Slow::findClosestHalfSpace(double* point, int base_plane_id, int partner_plane_id) {
...
vector<int> KerGen_Slow::findClosestHalfSpace(double* point, int base_plane_id, int partner_plane_id, int back_plane_id, Line line) {
	...
	// find each one that satisfy the condition
	for (int i = 0; i < halfSpaceSet.size(); i++) {
		if (ClosestPlaneToPoint_0D(i, base_plane_id, partner_plane_id, back_plane_id, line, candidate_point)) {
			closest_plane_ids.push_back(i);
...
vector<int> KerGen_Slow::orderFromFrontToBack(double* point, int base_plane_id, int partner_plane_id, vector<int> latest_plane_ids) {
	...
	for (int i = 0; i < latest_plane_ids.size(); i++) {
		...
		if (scalar > EPSILON) {	// if it falls behind the partner plane, recompute the test point by taking the reverse direction of the common line direction
			for (int j = 0; j < 3; j++)
				test_point[j] = point[j] - lines[i].directionVector[j];
		}
		/*
				double _PA[4] = { halfSpaceSet[partner_plane_id].point1[0], halfSpaceSet[partner_plane_id].point1[1], halfSpaceSet[partner_plane_id].point1[2], 0.0 };
				double _PB[4] = { halfSpaceSet[partner_plane_id].point2[0], halfSpaceSet[partner_plane_id].point2[1], halfSpaceSet[partner_plane_id].point2[2], 0.0 };
				double _PC[4] = { halfSpaceSet[partner_plane_id].point3[0], halfSpaceSet[partner_plane_id].point3[1], halfSpaceSet[partner_plane_id].point3[2], 0.0 };
				double _QQ[4] = { test_point[0], test_point[1], test_point[2], 0.0 };

				double _rr = geompred::orient3d(_PA, _PB, _PC, _QQ);
				if (_rr < -EPSILON) {
					for (int j = 0; j < 3; j++)
						test_point[j] = point[j] - lines[i].directionVector[j];
				}
		*/
		...
		/*
				for (int j = 0; j < latest_plane_ids.size(); j++) {
					...
					double _PA[4] = { halfSpaceSet[reference_plane_id].point1[0], halfSpaceSet[reference_plane_id].point1[1], halfSpaceSet[reference_plane_id].point1[2], 0.0 };
					double _PB[4] = { halfSpaceSet[reference_plane_id].point2[0], halfSpaceSet[reference_plane_id].point2[1], halfSpaceSet[reference_plane_id].point2[2], 0.0 };
					double _PC[4] = { halfSpaceSet[reference_plane_id].point3[0], halfSpaceSet[reference_plane_id].point3[1], halfSpaceSet[reference_plane_id].point3[2], 0.0 };
					double _QQ[4] = { test_point[0], test_point[1], test_point[2], 0.0 };
...
int KerGen_Slow::recordFoundVertex(double* vertex) {
	...
	if (vertex_id == -1) {
		...
		for (int i = 0; i < halfSpaceSet.size(); i++)
			if (scalar[i] > EPSILON) {	// not inside the kernel
				...
				delete[] scalar;
				return -i;	// return the id of the i^th plane in negative not to make confusion with a regular kernel vertex id
			}
		delete[] scalar;
...
bool KerGen_Slow::addIntoProcessQueueAfterCheck(int plane1_id, int plane2_id, int plane3_id, int base_plane_id, double* point) {
	...
	if (is_valid) {	// if not parallel then we can safely add this neighborhood into queue if there is no record related with plane1 id
		if (faceProcess[plane1_id] == ProcessColor_Slow::RED) {
			faceProcess[plane1_id] = ProcessColor_Slow::WHITE;
			tobeProcessedFaceIds.push(plane1_id);

			Line line(point, line1_direction);
			vertexPartnerships[plane1_id].push_back(VertexPartners_Slow(plane2_id, base_plane_id, line));
		}
		is_valid = true;
	}
...
bool KerGen_Slow::ClosestPlaneToPoint_3D(int candidate_plane_id, double* point, double* candidate_point) {
	...
	/*
	for (int i = 0; i < halfSpaceSet.size(); i++) {
		double _PA[4] = { halfSpaceSet[i].point1[0], halfSpaceSet[i].point1[1], halfSpaceSet[i].point1[2], 0.0 };
		double _PB[4] = { halfSpaceSet[i].point2[0], halfSpaceSet[i].point2[1], halfSpaceSet[i].point2[2], 0.0 };
		double _PC[4] = { halfSpaceSet[i].point3[0], halfSpaceSet[i].point3[1], halfSpaceSet[i].point3[2], 0.0 };
		double _QQ[4] = { candidate_point[0], candidate_point[1], candidate_point[2], 0.0 };
	...
	*/
	...
	/* Orient3d by CGAL
	for (int i = 0; i < halfSpaceSet.size(); i++) {
		CGAL::Sign sign = CGAL::orientation(
			CPoint(halfSpaceSet[i].point1), CPoint(halfSpaceSet[i].point2), CPoint(halfSpaceSet[i].point3), CPoint(candidate_point));
...
bool KerGen_Slow::ClosestPlaneToPoint_2D(int candidate_plane_id, int base_plane_id, double* point, double* candidate_point) {
...
bool KerGen_Slow::ClosestPlaneToPoint_1D(int candidate_plane_id, int base_plane_id, int partner_plane_id, Line line, double* candidate_point) {
...
bool KerGen_Slow::ClosestPlaneToPoint_0D(int candidate_plane_id, int base_plane_id, int partner_plane_id, int back_plane_id, Line line, double* candidate_point) {
...
void KerGen_Slow::findInitialPoint_1(double* point) {
